...
urlpatterns = [
    ...
    url(r'^login/reset_password/$', password_reset, {'template_name': 'accounts/display_password_reset.html'}, name='password_reset'),
    url(r'^login/reset_password/done/$', password_reset_done, {'template_name': 'accounts/display_password_reset_done.html'}, name='password_reset_done'),
    url(r'^login/reset_password/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', password_reset_confirm, {'template_name': 'accounts/display_password_reset_confirm.html'}, name='password_reset_confirm'),
    url(r'^login/reset_password/complete/$', password_reset_complete, {'template_name': 'accounts/display_password_reset_complete.html'}, name='password_reset_complete'),
